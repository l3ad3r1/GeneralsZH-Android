...
// GeneralsX @refactor android-port 08/02/2026 The Oboe/ALSOFT setup moved to
// Core/Main/SageAndroidBootstrap.cpp, shared with the Generals engine.
#include <cctype>
#include <cstring>
#include <cstdio>
...
int main(int argc, char* argv[])
{
...
#if defined(__ANDROID__)
	// GeneralsX @refactor android-port 08/02/2026 Moved to Core/Main/
	// SageAndroidBootstrap.cpp so the Generals engine can run the identical
	// startup. Two mains each carrying their own copy is exactly how the two
	// FFmpeg stub layers drifted apart and cost weeks of silent audio.
	...
	try {
		...
		// double-deliver finger 1 and fight the two-finger pan logic. Applies to
		// every mobile target, so it stays here rather than in the Android file.
		SDL_SetHint(SDL_HINT_TOUCH_MOUSE_EVENTS, "0");

		// GeneralsX @refactor android-port 08/02/2026 The Android-specific hints
		// (mouse-to-touch off, forced landscape, BACK trapped so right-click
		// works) live in Core/Main/SageAndroidBootstrap.cpp so the Generals
		// engine gets exactly the same set. No-op off Android.
...
#ifdef SAGE_MOBILE
		// GeneralsX @refactor android-port 08/02/2026 Moved to SageMobileInput so
		// the Generals engine gets it too; without it that build letterboxed.
